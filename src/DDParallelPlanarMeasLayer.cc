...
Int_t DDParallelPlanarMeasLayer::CalcXingPointWith(const TVTrack  &hel,
  ...
  // streamlog_out( DEBUG1 ) << " in DDParallelPlanarMeasLayer::CalcXingPointWith - mode: " << mode 
  // 			  << " - eps: " << eps << std::endl ;
  ...
  trkParam.setTrackParameters( aidaTT::Vector5( omega , tanl, phi0_lcio , d0,  z0 )  ) ; 
  ...
  if( foundIntersect ){
    ...
    streamlog_out( DEBUG1 ) << " ++++  intersection found for surface : " << _surf << std::endl 
			    << "       at s = " << s << std::endl
			    << "       xx   = ( " << xx.X() << ", " << xx.Y() << ", " << xx.Z() << ") "
			    <<  std::endl ;
    ...
    // streamlog_out( DEBUG0 ) << " ++++ no intersection found for surface : " << _surf << std::endl ;
  ...
  // // streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: Xc = " << this->GetXc().x() << " Yc = " << this->GetXc().y() << " Rc = " << this->GetXc().Perp() << " Phi = " << this->GetXc().Phi() << std::endl;
  // // streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: x1 = " << x1 << " y1 = "  << y1 << " R " << TVector3(x1,y1,0).Perp() << " phi = " << TVector3(x1,y1,0).Phi() << std::endl ; 
  // // streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: x2 = " << x2 << " y2 = "  << y2 << " R " << TVector3(x2,y2,0).Perp() << " phi = " << TVector3(x2,y2,0).Phi() << std::endl ; 
  ...
  // //             streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: Line segment doesn't intersect and is outside the circle " << std::endl;
  ...
  // //             streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: 1st solution u = " << u1 << " : " << u1 * dx << " " << u1 * dy << std::endl ;  
  // //             streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: 2nd solution u = " << u2 << " : " << u2 * dx << " " << u2 * dy << std::endl ;
  // //             streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: 1st solution x:y = " << x_ins1 << " : " << y_ins1 << std::endl ;
  // //             streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: 2nd solution x:y = " << x_ins2 << " : " << y_ins2 << std::endl ;
  ...
  // //   streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: Line segment doesn't intersect and is inside the circle " << std::endl;
  ...
  // //   streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: Only one possible solution" << std::endl;
  // //   streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: 1st solution u = " << u1 << " : " << u1 * dx << " " << u1 * dy << std::endl ;  
  // //   streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: 2nd solution u = " << u2 << " : " << u2 * dx << " " << u2 * dy << std::endl ;
  ...
  // //     //                        streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: take 1st solution x:y:z = " << x_ins << " : " << y_ins ;
  // //   }
  // //   else{ // use u2
  ...
  // //     //                        streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: take 2nd solution x:y:z = " << x_ins << " : " << y_ins ;
  // //   }
  ...
  // // 	streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: take 1st solution " << std::endl;
  // //       x_ins = x_ins1;
  // //       y_ins = y_ins1;
  // //       s_ins = s_1;
  // //     }
  // //     else {
  // // 	streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith: take 2nd solution " << std::endl;
  // //       x_ins = x_ins2;
  // //       y_ins = y_ins2;
  // //       s_ins = s_2;
  // //     }
  ...
  // // streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith mode = " << mode << " dr = " << dr << " dz = " << z0 << " x = " << ref_point.x() << " y = "<< ref_point.y() << " z = " << ref_point.z() << " r = " << ref_point.Perp() << std::endl;
  
  // // TVector3 xx_n;
  // // int cuts = TVSurface::CalcXingPointWith(hel, xx_n, phi, mode, eps);
  // // streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith from Newton: cuts = " << cuts << " x = " << xx_n.x() << " y = "<< xx_n.y() << " z = " << xx_n.z() << " r = " << xx_n.Perp() << " phi = " << xx_n.Phi() << " dphi = " <<  phi << " " << this->TVMeasLayer::GetName() << std::endl;
  
  // // xx.SetXYZ(x_ins, y_ins, z_pca + s_ins * tanl);
  
  // // phi = -s_ins * omega ;
  
  streamlog_out(DEBUG0) << "DDParallelPlanarMeasLayer::CalcXingPointWith:on surface:" <<  IsOnSurface(xx) 
