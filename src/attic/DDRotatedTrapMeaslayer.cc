...
DDRotatedTrapMeaslayer::DDRotatedTrapMeaslayer(TMaterial &min,
                                                 TMaterial &mout,
...
TKalMatrix DDRotatedTrapMeaslayer::XvToMv(const TVector3 &xv) const
{
...
TVector3 DDRotatedTrapMeaslayer::HitToXv(const TVTrackHit &vht) const
{
...
void DDRotatedTrapMeaslayer::CalcDhDa(const TVTrackHit &vht,
                                       const TVector3   &xxv,
...
Bool_t DDRotatedTrapMeaslayer::IsOnSurface(const TVector3 &xx) const
{
...
DDVTrackHit* DDRotatedTrapMeaslayer::ConvertLCIOTrkHit( EVENT::TrackerHit* trkhit) const {
  ...
  streamlog_out(DEBUG1) << "DDRotatedTrapMeaslayer::ConvertLCIOTrkHit DDPlanarHit created" 
  << " u = "  <<  x[0]
  << " v = "  <<  x[1]
  << " du = " << dx[0]
  << " dv = " << dx[1]
