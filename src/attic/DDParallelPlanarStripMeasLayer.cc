...
TKalMatrix DDParallelPlanarStripMeasLayer::XvToMv(const TVector3 &xv) const
{
 
//  streamlog_out(DEBUG0) << "\t DDParallelPlanarStripMeasLayer::XvToMv: "
//  << " x = " << xv.X() 
//  << " y = " << xv.Y() 
//  << " z = " << xv.Z() 
//  << std::endl;
...
TVector3 DDParallelPlanarStripMeasLayer::HitToXv(const TVTrackHit &vht) const
{

//  streamlog_out(DEBUG0) << "\t DDParallelPlanarStripMeasLayer::HitToXv: "
//  << " vht(0,0) = " << vht(0,0);
//  if (DDPlanarStripHit_DIM == 2) {
...
void DDParallelPlanarStripMeasLayer::CalcDhDa(const TVTrackHit &vht,
                             const TVector3   &xxv,
...
//  streamlog_out(DEBUG0) << "\t DDParallelPlanarStripMeasLayer::CalcDhDa: dudx = " << dudx << " dudy = " << dudy << " dudz = " << dudz << std::endl ;
//  if (DDPlanarStripHit_DIM == 2) streamlog_out(DEBUG0) << "\t DDParallelPlanarStripMeasLayer::CalcDhDa: dvdx = " << dvdx << " dvdy = " << dvdy << " dvdz = " << dvdz << std::endl ;
...
DDVTrackHit* DDParallelPlanarStripMeasLayer::ConvertLCIOTrkHit( EVENT::TrackerHit* trkhit) const {
  
  EVENT::TrackerHitPlane* plane_hit = dynamic_cast<EVENT::TrackerHitPlane*>( trkhit ) ;

  if( plane_hit == NULL )  { 
    streamlog_out(ERROR) << "DDParallelPlanarStripMeasLayer::ConvertLCIOTrkHit dynamic_cast to DDPlanarStripHit failed " << std::endl; 
  ...
  Double_t  x[DDPlanarStripHit_DIM] ;
  Double_t dx[DDPlanarStripHit_DIM] ;
  ...
  streamlog_out(DEBUG1) << "DDParallelPlanarStripMeasLayer::ConvertLCIOTrkHit DDPlanarStripHit created" 
